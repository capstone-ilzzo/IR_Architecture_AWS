...
def upload_s3(instance_id,case_time):
    os.system(f"aws s3 cp /home/ec2-user/volatility3/result.txt s3://ilzzo-forensic-artifact/{instance_id}/{case_time}/result.txt")
...
def download_lime_file(instance_id,case_time):
    command = f'aws s3 cp s3://ilzzo-forensic-artifact/{instance_id}/{case_time}/memory/{instance_id}.lime /home/ec2-user/volatility3/tmp.lime'    
    os.system(command)
 
def command_artifact(output_file):
    s3 = f'aws s3 cp s3://ilzzo-forensic-artifact/{instance_id}/{case_time}/command/'
    command_output = [
        'date.txt',
        'uname_output.txt',
        'ifconfig_output.txt',
